"""Existing extraction workflow, available alongside the audit workspace."""
...
from .export.columns import build_columns, record_row
from .models import RecordStatus
from .pipeline import Pipeline
from .sources import SourceError, all_sources

ROOT = Path.cwd()
PROJECT_DIR = ROOT / "projects"
STATUS_COLOURS = {"VERIFIED": "#C6EFCE", "PARTIALLY_VERIFIED": "#DDEBF7", "UNVERIFIED": "#EDEDED",
                  "NEEDS_REVIEW": "#FFC7CE", "EXCLUDED": "#FFEB9C"}
...
def render():
    ...
    with st.sidebar:
        ...
        run_clicked = st.button("Run extraction", type="primary", width="stretch")
        st.divider()
    ...
    with tab_data:
        ...
        st.dataframe(view.style.map(colour_status, subset=["Verification status"]), width="stretch", height=520)
    ...
    with tab_files:
        for kind, path in result.outputs.items():
            if str(path).startswith(("https://", "http://")):
                st.link_button(f"Open {kind}", str(path))
                continue
            p = Path(path)
